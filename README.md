...
Current source version: `0.4.2-experimental`.
...
1. Add the pinned experimental repository marketplace:
   ...
   ```bash
   codex plugin marketplace add iammedved/Pinmind --ref v0.4.2-experimental
   ```
...
The repository marketplace is separate from OpenAI's universal Plugins Directory. Pinning `v0.4.2-experimental` selects this exact prerelease; omit `--ref` only when you intentionally want the latest repository state.
